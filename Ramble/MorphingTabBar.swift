...
//  MorphingTabBarTest
//
//  Created by Jean-Marc Boullianne on 3/1/20.
        ...
        VStack(alignment: .center, spacing: 0) {
            ZStack{
                ForEach(self.views.indices) { i in
                    self.views[i].onPreferenceChange(MorphTabItemPreferenceKey.self) { image in
                        self.tabItems.insert(TabItem(uuid: UUID(), pos: i, image: image), at: 0)
                    }.zIndex(i == self.index ? Double(self.views.count) : 0)
                    .padding()
                }
            }
            ZStack {
